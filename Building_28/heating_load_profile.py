...
building = 'Haus 28'
building_eng = 'Building 28'
label_Hl = 'Haus_28'
Ta, GTN, GTO, GTH, GTS, GTW = weather_data()

#  Fensterflächen
f1 = 3.78  # einzelne Fensterfläche
f2 = 1.2051

AFN = np.array([0,0])
AFO = np.array([41*f1, 32*f2])
AFS = np.array([8*f1, 0])
AFW = np.array([31*f1,32*f2])
AFH = np.array([0, 0])
...
# Gebäude:
Traufhoehe = np.array([3.5,2])
Grundflaeche = np.array([1546.46, 463.94])
Gebaeudeumfang = np.array([194.6, 170.6])
...
plt.ylim(0, None)
plt.xlim(0, 8761)
p.set_title(f"Heat load profile {building_eng}", fontsize = 30)
...
plt.ylim(0, None)
plt.xlim(0, 8761)
p.set_title(f"Load duration curve {building_eng}", fontsize = 30)
p.set_xlabel("Time (hours)", fontsize = 20)
p.set_ylabel("Power (KW)", fontsize = 20)
